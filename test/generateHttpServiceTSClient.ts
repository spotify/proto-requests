...
const generateCustomClasses = (fileLocation) => {
    ...
    new protoRequest.HttpServiceTSClient(fileLocation, clientConfig, (err, client) => {
        if (err) {
        ...
        Object.keys(customClasses).forEach(className => {
            ...
            const outputFileName = `${config.clientOutputDir}${outputPath}${className}.ts`;
...
const generateHttpServiceClient = (fileLocation) => {
    ...
    new protoRequest.HttpServiceTSClient(fileLocation, clientConfig, (err, client) => {
        ...
        const outputFileName = `${config.clientOutputDir}${shortServiceName}.ts`;
